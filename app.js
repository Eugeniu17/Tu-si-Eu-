...
function renderChapter(){
 ...
 const labels=['Deschide',d.magicLabel||'Atmosferă','Gest','Întrebare','Alegere','Rugăciune','Sigiliu'];
...
function renderStage(){
 ...
 if(stage===5)root.innerHTML=card('Trei motive de rugăciune',`${fastingBlock(d)}<p>Scrie starea, nevoia și felul în care celălalt te poate susține. Nu trebuie să pari puternic(ă).</p><div class="prayers">${d.prayer.map((p,i)=>`<div class="prayer-row"><span class="prayer-number">${i+1}</span><div><b>${esc(p)}</b><textarea class="textarea" data-prayer="${i}" placeholder="Cum mă simt? Ce cer lui Dumnezeu? Cum mă poți susține?">${esc(answer().prayers?.[i]||'')}</textarea></div></div>`).join('')}</div><button class="primary" id="savePrayer">Păstrează motivele noastre</button>`);
